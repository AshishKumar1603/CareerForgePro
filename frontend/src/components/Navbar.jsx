...
function Navbar({
  ...
  return (
    <nav
      className={`sticky top-0 z-50 flex items-center justify-between px-4 sm:px-8 py-3 bg-white/95 dark:bg-gray-900/95 shadow-sm dark:shadow-gray-800/50 backdrop-blur-md border-b border-gray-200/50 dark:border-gray-700/50 transition-all duration-300 ${className}`}
    >
      {/* Left: Back + Brand */}
      <div className="flex items-center gap-3">
        {showBack && onBack && (
      ...
      <div className="flex items-center gap-2">
        {!showBack && (
          <div className="hidden md:flex gap-1 text-sm font-medium text-gray-600 dark:text-gray-300 mr-2">
            <a
              href="#features"
              className="hover:text-violet-600 dark:hover:text-violet-400 px-3 py-2 rounded-lg hover:bg-violet-50 dark:hover:bg-violet-900/30 transition-all"
            >
              Features
            </a>
            <a
        ...
        {toggleTheme && (
          <button
            onClick={toggleTheme}
            className="flex items-center justify-center w-9 h-9 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-200 text-gray-500 dark:text-gray-400"
